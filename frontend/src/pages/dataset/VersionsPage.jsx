...
export default function VersionsPage({ datasetId: propDatasetId }) {
  ...
  const confirmDelete = async () => {
    await db.datasetVersions.delete(deleteConfirm);
    setVersions(v => v.filter(x => x.id !== deleteConfirm));
      ...
      <div style={styles.left}>
        <div style={styles.header}>
          ...
          <div>
            ...
            <Button size="sm" variant="primary" onClick={() => { if (selected) exportVersion(selected); }} disabled={!selected}>Export</Button>
          </div>
        </div>
              ...
              return (
                <div>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    ...
                    <Button variant="link" size="sm" onClick={() => setDeleteConfirm(selected)} style={{ color: themeColors.danger }} title="Delete"><Trash2 size={16} /></Button>
                  </div>
        ...
        {deleteConfirm && (
          <div style={{ marginTop: 12, ...styles.card }}>
            ...
            <div style={{ marginTop: 8, color: themeColors.subtleText }}>This will permanently remove the version configuration (annotations / images are not deleted).</div>
            ...
            <ListGroup.Item key={v.id} action style={{ background: selected === v.id ? themeColors.primary : themeColors.cardBg, color: selected === v.id ? '#fff' : themeColors.text, borderBottom: `1px solid ${themeColors.border}` }} onClick={() => { setSelected(v.id); setMode('view'); }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                  ...
                  <Button variant="link" size="sm" onClick={(e)=>{ e.stopPropagation(); setDeleteConfirm(v.id); }} title="Delete"><Trash2 size={14} color={themeColors.danger} /></Button>
                </div>
              </div>
            </ListGroup.Item>
          ))}
